...
\hypersetup{
    ...
    pdfkeywords={Bitcoin, illicit transaction detection, graph neural networks, GraphSAGE, GCN, GAT, tabular machine learning, temporal generalization, class imbalance}
}
...
\begin{itemize}
    ...
    \item \textbf{Random Forest (RF):} Bagged ensemble of 100 decision trees (\texttt{n\_estimators=100}, \texttt{max\_depth=15}, \texttt{min\_samples\_split=5}, \texttt{min\_samples\_leaf=2}) \cite{breiman2001random}.
    ...
    \item \textbf{Multi-Layer Perceptron (MLP):} Deep neural network with hidden dimensions $[128, 64]$, ReLU activations, Dropout ($p=0.2$), Batch Normalization, trained via Adam ($lr=0.001$, weight decay $10^{-4}$, batch size 256, maximum epochs 40, early stopping patience 7).
\end{itemize}
...
\begin{itemize}
    ...
    \end{equation}
\end{itemize}
The primary GNN benchmark configuration uses two message-passing layers for all three architectures; a separate depth ablation evaluates one-, two-, and three-layer variants. GNNs employ 128 hidden channels, Dropout ($p=0.3$), and Adam optimizer ($lr=0.001$, weight decay $10^{-4}$). GAT utilizes 8 attention heads with 16 channels per head in the multi-head layer ($8 \times 16 = 128$). Hyperparameter details are provided in Table II.
...
Random Forest & Bagged Trees & 100 trees, depth 15, split 5, leaf 2 \\
XGBoost & Boosting & 100 trees, depth 6, lr 0.1, sub 0.8 \\
MLP & Neural Net & Layers [128, 64], Dropout 0.2, Adam, bs 256 \\
GCN & Graph Conv & 2 layers, 128 hidden, Adam ($10^{-3}$) \\
GraphSAGE & Inductive GNN & 2 layers, 128 hidden, Mean aggregate \\
GAT & Attention GNN & 2 layers, 128 hidden, 8 heads (16/head) \\
\bottomrule
\end{tabular}
}
}
...
During model optimization, the GNN trainer executes full-batch message passing on Timesteps 1--34 with Early Stopping (patience = 15 epochs) monitoring validation PR-AUC on Timesteps 35--39. When validation PR-AUC ceases to improve for 15 evaluations, training terminates and the checkpoint achieving the highest validation PR-AUC is restored. Next, decision threshold optimization is conducted on the restored model to maximize illicit-class F1 on the validation set ($\tau^* = \arg\max \text{F1}_{\text{val}}$). This optimal threshold is then frozen and evaluated on the blind test holdout partition (Timesteps 40--49).
...
Table VII and Fig. 6 present the ablation comparing the full $100\%$ topology (retaining unlabeled nodes) against an induced subgraph of labeled nodes only (removing unknown nodes), defining $\Delta\text{F1} = \text{Retained} - \text{Removed}$:
\begin{itemize}
    \item For GAT, retaining unknown nodes improved test illicit F1 from $0.2918$ to $0.3308$ ($\Delta\text{F1} = +0.0390$), indicating that attention mechanisms successfully filter uninformative connections.
    \item For GCN, retaining unknown nodes degraded test illicit F1 from $0.4555$ to $0.2589$ ($\Delta\text{F1} = -0.1966$).
    \item For GraphSAGE, retaining unknown nodes degraded test illicit F1 from $0.4132$ to $0.2822$ ($\Delta\text{F1} = -0.1310$), indicating that the effect of unlabeled graph context is architecture-dependent under the evaluated setup.
\end{itemize}
...
Table XI details the error taxonomy and graph connectivity for the validation-selected GraphSAGE model using the Local 93-feature configuration across the $11,184$ test nodes ($402$ TP, $8,165$ TN, $2,383$ FP, $234$ FN):
\begin{itemize}
...
\begin{table}[htbp]
\centering
\caption{Error and Graph-Structural Taxonomy for Validation-Selected GraphSAGE (Local 93 Features, Test Set, $N = 11,184$)}
...
\subsection{Temporal Non-Stationarity and Timestep 43 Regime Change}
The sharp change in illicit prevalence around timestep 43 coincides with a substantial reduction in per-timestep illicit prevalence ($11.10\% \to 1.75\%$) and a corresponding degradation in model F1 (dropping from $0.5234$ to $0.0718$). Because the dataset provides anonymized discrete timesteps rather than verified event annotations, the cause of this regime change cannot be established from the benchmark alone.

\subsection{Practical Implications for Transaction Forensics}
In this evaluated benchmark, well-tuned tree ensembles (Random Forest and XGBoost) combined with 1-hop pre-aggregated topological features demonstrated strong detection capability and high parameter stability across seeds. However, establishing production readiness in live compliance systems requires additional evaluation of inference latency, probability calibration, continuous temporal drift, operational deployment constraints, and streaming data quality.
...
Temporal distribution shift and sudden changes in illicit transaction prevalence introduce substantial concept drift across chronological snapshots \cite{gama2014survey, quinonero2009dataset}.
